...
case "$choice" in 
  d|D ) if [ "$DT_DEL" == "Y" ] # If Docker Cleanup variable is set to Y
        then
              ...
              docker rmi $(docker images --filter=reference="kicbase/stable:*" -q) # Remove dovker images
        fi ;
        echo "*** Using Docker... ***"; minikube start --driver=docker;;
  k|K ) echo "*** Using KVM2... ***"; minikube start --driver=kvm2;;
  * ) echo "*** Invalid option selected, exiting... ***"; exit;;
esac
...
case "$choice" in 
  y|Y ) echo "*** Creating hello-node Namespace. ***"; 
        ...
        kitty -- /bin/sh -c "minikube service -n hello-node hello-node; echo Hello Node launched - Press enter to stop; read _" &;;
  n|N ) echo "*** Continuing... ***";;
...
case "$choice" in 
  y|Y ) echo "*** Launching MiniKube Dashboard... ***"; 
        kitty -- /bin/sh -c "minikube dashboard; echo Dashboard launched - Press enter to stop; read _" &;;
